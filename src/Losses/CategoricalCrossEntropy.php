...
class CategoricalCrossEntropy extends Loss implements IGrad
{
    ...
    function __invoke(\NDArray $true, Variable $pred): Variable
    {
        ...
        $out = Variable::divide(Variable::multiply(Variable::sum(Variable::multiply($true, Variable::log($out))), $m_ones), Variable::fromArray(count($true->getArray())));
    ...
    {
        $denominator = nd::clip(($op->getArgs()[1]->getArray() * (1 - $op->getArgs()[1]->getArray())), $this->epsilon, 1 - $this->epsilon);
